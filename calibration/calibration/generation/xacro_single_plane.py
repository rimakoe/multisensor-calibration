...
xacro_single_plane_bottom.world.add_child(
    create_plane(
        ...
        transform=SE3(translation=np.array([6.0, 0.0, 0.1]), rotation=Rotation.from_euler("xyz", [0.0, 0.0, 0.0], degrees=True)),
...
xacro_single_plane_front.world.add_child(
    create_plane(
        name="p" + str(plane_id),
        size=np.array([3.0, 3.0, 0.001]),
        transform=SE3(translation=np.array([5.0, 0.0, 1.5]), rotation=Rotation.from_euler("xyz", [0.0, -90.0, 0.0], degrees=True)),
        marker_descriptions=create_nxm_marker_descriptions(n, m),
    )
)
